...
class Twitch:
    ...
    def start(self, delay = 5):
        ...
        while not self._stop:
            ...
            #check for used-to online streams
            filter_true = [k for k,v in self.ch_table.iteritems() if v==True]
            for stream in data:
                channel = str(stream["channel"]["name"])
                if (self.ch_table[channel] != True):
                    #went live
                    ClUs.user.notify(channel)
                    self.ch_table[channel] = True
                else:
                    #still online
                    filter_true.remove(channel)

            #set elem to false if stream is off
            for ch in filter_true:
                #went offline
                self.ch_table[ch] = False
